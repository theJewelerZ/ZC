...
export default async function ConsultationDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const sourceProject = consultation.source_project_id
    ? (await client.from("projects").select("id,internal_name,slug,publication_status").eq("id", consultation.source_project_id).maybeSingle()).data
    : null;
