...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <main className="admin-shell" id="main-content">
    <div className="admin-heading"><div><p className="eyebrow">Founder dashboard</p><h1>Consultations</h1><p>Private inquiry records. Email is a notification channel; this list is the system of record.</p></div><div className="admin-security-actions"><Link className="button button-outline" href="/admin/set-password">Set password</Link><form action="/auth/signout" method="post"><button className="button button-outline" type="submit">Sign out</button></form></div></div>
